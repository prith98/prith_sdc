...
function IndividualQandA() {
  ...
  const getAnswers = function (id) {
    axios.get("/qa/questions/" + id.toString() + "/answers").then((results) => {
  ...
  const updateAHelpful = function (e) {
    ...
    if (!answerIDs[aID]) {
      axios
        ...
        .then((results) => {
          console.log(
            "Successfully marked answer " + aID.toString() + " as helpful"
          );
        });
    } else {
  ...
  const fillAnswerIDs = function () {
    ...
    for (var i = 0; i < qIDArray.length; i++) {
      let qID = qIDArray[i];
      axios.get("/qa/questions/" + qID + "/answers").then((results) => {
        let resultsArray = results.data.results;
  ...
  return (
    <div id="IndividualQA">
      ...
      {limitQuestions && reportAnswerIDs ? (
          ...
          let finalAnswers = answerArray.map((oneAnswer) => {
            return (
              <div key={oneAnswer.id} id="totalAnswer">
                ...
                <div className="answerBottomText">
                  by {oneAnswer.answerer_name}, {oneAnswer.date.slice(0, 10)} |
                  Helpful?{" "}
                  <span
                    className="helpfulYes"
                    data-id={oneAnswer.id}
                    onClick={updateAHelpful}
                  >
                    <u>Yes</u>
                  </span>
                  ({oneAnswer.helpfulness}) |{" "}
                  <span
                    data-id={oneAnswer.id.toString()}
                    className="reportAnswer"
                    onClick={reportAnswer}
                  >
                    <u>
                      {reportAnswerIDs[oneAnswer.id] ? "Reported" : "Report"}
                    </u>
                  </span>
                </div>
              </div>
          ...
          return (
            <div key={oneQuestion.question_id} className="individualQA">
              <div id="questionAsked">
                ...
                <span id="questionAskedInfo">
                  {" "}
                  asked by: {oneQuestion.asker_name} | Date Asked:{" "}
                  {oneQuestion.question_date.slice(0, 10)} | Helpful?{" "}
                  <span
                    className="helpfulYes"
                    data-id={oneQuestion.question_id}
                    onClick={updateQHelpful}
                  >
                    <u>Yes</u>
                  </span>{" "}
                  ({oneQuestion.question_helpfulness}) |{" "}
                  <span
                    data-id={oneQuestion.question_id}
                    className="addAnswer"
                    onClick={updateQID}
              ...
              <div id="answers">
                {questionIDs2[oneQuestion.question_id]
                  ? finalAnswersSliced
                  : finalAnswers}
              </div>
              <div id="loadingButton">
                {questionIDs2[oneQuestion.question_id] ? (
                  <button
                    onClick={loadAnswers}
                    name={oneQuestion.question_id}
                    className="answerLoad"
                  >
                    LOAD MORE ANSWERS
                  </button>
                ) : null}
