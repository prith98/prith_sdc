...
function RightPanel() {
  ...
  if (currentStyleData["sale_price"] !== null) {
    var oldPrice = (
      <div style={{ textDecoration: "line-through" }}>
        ${currentStyleData["original_price"].split(".")[0]}
      </div>
    );
    var newPrice = (
      <div
      ...
      >
        ${currentStyleData["sale_price"].split(".")[0]}
      </div>
    );
    var salePriceDiv = (
      <div
  ...
  function addToCart() {
    if (select_size) {
      ...
      for (let key in currProdStyle["skus"]) {
        if (size === "XXL") {
          sku_id = Object.keys(currProdStyle["skus"])[
            Object.keys(currProdStyle["skus"]).length - 1
          ];
        }
  ...
  return (
    <div
      ...
      style={{
        display: extend === false ? "" : "none",
        webkitTransition: "display 1s ease",
      }}
    >
      <div
        style={{
      ...
      <div
        style={{
          fontWeight: "bold",
          color: "rgb(139,0,0)",
          display: selectSizeWarning === true ? "" : "none",
        }}
      >
        Please select size
      </div>
      <select
        onChange={changeSizeQuantity}
        className="selectsizetext"
      ...
      <select
        ...
        className="quantitycarttext"
        disabled={status}
        style={{ cursor: "pointer" }}
      >
        <option value={status === true ? "-" : 1}>
          {status === true ? "-" : 1}
        </option>
        {quantityList}
      </select>
