...
function Arrows(props) {
  const {
    ...
    setThumbnailCount,
    loadNextThumbnail,
    setLoadNextThumbnail,
    thumbnailIncrement,
    setThumbnailIncrement,
    slideIndex,
    setSlideIndex,
    extend,
    setExtend,
  ...
  function showSlides(n) {
    ...
    if (n >= mainPictures.length) {
      localSlideIndex = 0;
      setSlideIndex(0);
    }
    if (n < 0) {
      localSlideIndex = mainPictures.length - 1;
      setSlideIndex(mainPictures.length - 1);
    }
  ...
  function preventOutOfIndex(n, direction) {
    ...
    if (photos[0 + n] !== undefined && direction === "up") {
      ...
      photos[photos.length - 2 + n] !== undefined &&
      direction === "down"
    ) {
      setThumbnailIncrement(n);
    }
  ...
  return (
    <div>
      ...
      <a
        ...
        style={{
          display: zoomEnabled === true ? "none" : "",
          right: extend === true ? "-388px" : "60px",
          webkitTransition: "right 1s ease",
        }}
        onClick={() => {
          plusSlides(1);
        }}
