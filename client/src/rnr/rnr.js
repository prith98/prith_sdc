...
function Rnr() {
  ...
  useEffect(() => {
    ...
    Axios.get(`/reviews/meta?product_id=${currentProductId}`).then(
      (response) => {
        setProductRatings(response);
      }
    );
    Axios.get(`/reviews?product_id=${currentProductId}&sort=newest`).then(
      (response) => {
        setSortedNewest(response.data.results);
      }
    );
    Axios.get(`/reviews?product_id=${currentProductId}&sort=helpful`).then(
      (response) => {
        setSortedHelpful(response.data.results);
      }
    );
    Axios.get(`/reviews?product_id=${currentProductId}&sort=relevant`).then(
      (response) => {
    ...
    return (
      ...
      >
        <div id="rnr" className="ratingsAndReviews">
          <div className="ratingsAndReviews-title">Ratings and Reviews</div>
