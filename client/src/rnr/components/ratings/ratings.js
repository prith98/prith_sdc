...
function Ratings() {
  ...
  let totalRatingsWeighted =
    oneStarRatingsWeighted +
    twoStarRatingsWeighted +
    threeStarRatingsWeighted +
    fourStarRatingsWeighted +
    fiveStarRatingsWeighted;
  ...
  let recommendedPercentage =
    (recommendedTrue / (recommendedTrue + recommendedFalse)) * 100;
    ...
    if (recommendedPercentageAsAString[3] === "9") {
      if (recommendedPercentageAsAString[1] === "9") {
        if (recommendedPercentageAsAString[0] === "9") {
          ...
          recommendedPercentageAsAString =
            (Number(recommendedPercentageAsAString[0]) + 1).toString() + "0%";
        }
      } else {
        recommendedPercentageAsAString =
          recommendedPercentageAsAString[0] +
          (Number(recommendedPercentageAsAString[1]) + 1).toString() +
          "%";
      }
    } else {
      recommendedPercentageAsAString =
        recommendedPercentageAsAString.slice(0, 3) +
        (Number(recommendedPercentageAsAString[3]) + 1).toString() +
        "%";
    }
  } else {
    recommendedPercentageAsAString =
      recommendedPercentageAsAString.slice(0, 4) + "%";
  }

  if (recommendedPercentageAsAString[3] === "0") {
    recommendedPercentageAsAString =
      recommendedPercentageAsAString.slice(0, 2) + "%";
  }
  ...
  for (let i = 5; i > 0; i--) {
    ...
    starRatingsArray.push({
      stars: stars,
      numberOfRatings: productRatings.data.ratings[i.toString()],
      percentage: starPercentageRounded,
    });
  }

  //Create star rating icon
  let numberToProduceStar;
  let overallProductRatingToTheHundreth =
    Math.round(overallProductRating * 100) / 100;
  ...
  let overallStarRatingAsAStringAfterDecimal =
    overallStarRatingAsAString.slice(1);

  if (Number(overallStarRatingAsAStringAfterDecimal) < 0.13) {
    numberToProduceStar = Number(overallStarRatingAsAStringFirstDigit);
  } else if (
    Number(overallStarRatingAsAStringAfterDecimal) >= 0.13 &&
    Number(overallStarRatingAsAStringAfterDecimal) < 0.38
  ) {
    numberToProduceStar = Number(overallStarRatingAsAStringFirstDigit) + 0.25;
  } else if (
    Number(overallStarRatingAsAStringAfterDecimal) >= 0.38 &&
    Number(overallStarRatingAsAStringAfterDecimal) < 0.63
  ) {
    numberToProduceStar = Number(overallStarRatingAsAStringFirstDigit) + 0.5;
  } else if (
    Number(overallStarRatingAsAStringAfterDecimal) >= 0.63 &&
    Number(overallStarRatingAsAStringAfterDecimal) < 0.88
  ) {
    ...
    <MainContext.Provider value={{ productRatings }}>
      <div className="ratings-container" style={{ padding: "10px" }}>
        <div className="ratings-number-and-stars">
          <div className="ratings-decimal">{overallProductRating}</div>
          <div className="ratings-overall-star" style={{ marginLeft: "15px" }}>
            {starRatingImage}
          </div>
        </div>
        <div
          className="ratings-recommend-percentage"
        ...
        <div className="ratings-sliders" style={{ marginTop: "10px" }}>
          {comfortRating !== null ? (
            <div className="ratings-comfort">
              <div
                className="ratings-comfort-text"
              ...
              <span
                style={{
                  display: "inline-block",
                  position: "absolute",
                  marginLeft: comfortSliderPosition,
                }}
              >
                &nabla;
              </span>
              <div
                className="comfort-descriptors"
                style={{ marginTop: "12px" }}
              >
                <span style={{ fontSize: "10px" }}>Poor</span>
                <span style={{ fontSize: "10px", marginLeft: "212px" }}>
                  Perfect
                </span>
              </div>
            </div>
          ...
          {fitRating !== null ? (
            <div className="ratings-fit" style={{ marginTop: "10px" }}>
              <div className="ratings-fit-text">Fit</div>
              <span
                className="fit-bar"
                style={{
                  position: "absolute",
          ...
          {lengthRating !== null ? (
            <div className="ratings-length" style={{ marginTop: "10px" }}>
              <div className="ratings-length-text">Length</div>
              <span
              ...
              <span
                style={{
                  display: "inline-block",
                  position: "absolute",
                  marginLeft: lengthSliderPosition,
                }}
              >
                &nabla;
              </span>
              <div className="length-descriptors" style={{ marginTop: "12px" }}>
                ...
                <span style={{ fontSize: "10px", marginLeft: "72px" }}>
                  Perfect
                </span>
                <span style={{ fontSize: "10px", marginLeft: "72px" }}>
                  Too Long
          ...
          {qualityRating !== null ? (
            <div className="ratings-quality" style={{ marginTop: "10px" }}>
              <div className="ratings-quality-text">Quality</div>
              <span
                className="quaility-bar"
                style={{
                  position: "absolute",
                  display: "inline-block",
                  marginTop: "4px",
              ...
              <span
                style={{
                  display: "inline-block",
                  position: "absolute",
                  marginLeft: qualitySliderPosition,
                }}
              >
                &nabla;
              </span>
              <div
                className="quality-descriptors"
                style={{ marginTop: "12px" }}
              >
                <span style={{ fontSize: "10px" }}>Poor</span>
                <span style={{ fontSize: "10px", marginLeft: "212px" }}>
